...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input, mask, kv=None, cache=None):
        ...
        if (q.shape[-2] > k.shape[-2]):
          k = torch.cat((k,torch.zeros(q.shape[0],q.shape[1],q.shape[2]-k.shape[2],q.shape[3]).cuda()),dim=-2)
        elif (k.shape[-2] > q.shape[-2]):
          q = torch.cat((q,torch.zeros(k.shape[0],k.shape[1],k.shape[2]-q.shape[2],k.shape[3]).cuda()),dim=-2)
